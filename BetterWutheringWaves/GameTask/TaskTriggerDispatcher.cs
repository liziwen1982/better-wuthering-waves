...
namespace BetterWutheringWaves.GameTask
{
    public class TaskTriggerDispatcher : IDisposable
    {
        ...
        public static TaskTriggerDispatcher Instance()
        {
            if (_instance == null)
            {
                throw new Exception("请先在启动页启动BetterWW，如果已经启动请重启");
            }
        ...
        public void Tick(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (GameCapture == null || !GameCapture.IsCapturing)
                {
                    ...
                    {
                        _logger.LogInformation("游戏已退出，BetterWW 自动停止截图器");
                    }
                ...
                // 检查游戏是否在前台
                var active = SystemControl.IsGameActive();
                if (!active)
                {
                    // 检查游戏是否已结束
                    if (TaskContext.Instance().SystemInfo.GameProcess.HasExited)
                    ...
                    if (pName != "BetterWW" && pName != "Client-Win64-Shipping")
                    {
                        maskWindow.Invoke(() => { maskWindow.Hide(); });
                    }
