...
public class SystemControl
{
    ...
    public static async Task<nint> StartFromLocalAsync(string path)
    {
        ...
        return FindGameHandle();
    }

    public static bool IsGameActiveByProcess()
    {
        var name = GetActiveProcessName();
        return name is "Client-Win64-Shipping";
    }
    ...
    {
        var handle = (nint)User32.FindWindow("UnityWndClass", "鸣潮");
        if (handle != 0)
        {
            return handle;
        }
        ...
        handle = (nint)User32.FindWindow("Qt5152QWindowIcon", "云·鸣潮");
        if (handle != 0)
        {
            return handle;
        }
    ...
    public static void ActivateWindow()
    {
        ...
        {
            throw new Exception("请先启动BetterWW");
        }
